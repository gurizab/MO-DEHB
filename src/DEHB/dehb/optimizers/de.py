...
class DE(DEBase):
    def __init__(self, cs=None, f=None, dimensions=None, pop_size=20, max_age=np.inf,
                 ...
                 budget=None, encoding=False, dim_map=None, max_model_params=2e7, **kwargs):
    ...
    def init_eval_pop(self, budget=None, eval=True, **kwargs):
        ...
        self.G = np.array([np.inf for i in range(self.pop_size)])
        ...
        for i in range(self.pop_size):
            ...
            self.G[i] = info["constraints"] if "constraints" in info else 0
    ...
    def selection(self, trials, budget=None, **kwargs):
        ...
        for i in range(len(trials)):
            ...
            if fitness <= self.fitness[i] and G <= self.G[i]:
                ...
                self.age[i] -= 1
                self.archive.append(trials[i])
                self.archive_fitness.append(fitness)
                self.archive_G.append(G)
    ...
    def check_n_params(self, vector, configspace=False):
        if not configspace:
            config = self.vector_to_configspace(vector=vector)
        else:
            config = vector
        ...
        if total_model_params <= self.max_model_params:
            return True
        else:
            return False
    ...
    def evolve_generation(self, budget=None, best=None, alt_pop=None, **kwargs):
        ...
        for j in range(self.pop_size):
            ...
            while not valid_offspring:
                ...
                valid_offspring = self.check_n_params(trial)
            trials.append(trial)
    ...
    def run(self, generations=1, verbose=False, budget=None, reset=True, **kwargs):
        ...
        for i in range(generations):
            ...
            if i % self.empty_archive_freq == 0 and len(self.archive) > 0:
                sorted_idx = np.argsort(self.fitness)[::-1]
                for i in sorted_idx[:-1]:
                    if len(self.archive) == 0:
                        break
                    else:
                        self.population[i] = self.archive.pop()
                        self.fitness[i] = self.archive_fitness.pop()
                        self.G[i] = self.archive_G.pop()
    ...
    def init_population(self, pop_size: int) -> List:
        if self.configspace:
            ...
            for i in range(pop_size):
                valid_offspring = False
                while not valid_offspring:
                    individual = self.cs.sample_configuration()
                    valid_offspring = self.check_n_params(individual, configspace=True)

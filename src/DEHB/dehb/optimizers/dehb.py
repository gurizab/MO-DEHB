...
sys.path.append("/home/guri/Desktop/AutoML_Project/automl-ss21-final-project-gurizab/src/DEHB/dehb")
from DEHB.dehb.optimizers import DE, AsyncDE
from DEHB.dehb.utils import SHBracketManager
...
class DEHBBase:
    def __init__(self, cs=None, f=None, dimensions=None, mutation_factor=None,
                 ...
                 boundary_fix_type='random', max_age=np.inf, max_model_params=2e7, **kwargs):
...
class DEHB(DEHBBase):
    def __init__(self, cs=None, f=None, dimensions=None, mutation_factor=0.5,
        ...
        super().__init__(cs=cs, f=f, dimensions=dimensions, mutation_factor=mutation_factor,
                         ...
                         max_age=max_age, max_model_params=max_model_params, **kwargs)
    ...
    def _init_subpop(self):
        ...
        for i, b in enumerate(self._max_pop_size.keys()):
            ...
            self.de[b].G = np.array([np.inf] * self._max_pop_size[b])
    ...
    def _acquire_config(self, bracket, budget):
        ...
        while not valid_offspring:
            ...
            valid_offspring = self.de[budget].check_n_params(config)
    ...
    def _get_next_job(self):
        ...
        if len(self.active_brackets) == 0 or \
            ...
            for _bracket in self.active_brackets:
                # check if _bracket is not waiting for previous rung final_results of same bracket
                # _bracket is not waiting on the last rung final_results
    ...
    def _fetch_results_from_workers(self):
        """ Iterate over futures and collect final_results from finished workers
        """
        if self.n_workers > 1 or isinstance(self.client, Client):
        ...
        for _, future in done_list:
            ...
            G = info["constraints"] if "constraints" in info else 0
            ...
            if fitness <= self.de[budget].fitness[parent_id] and G <= self.de[budget].G[parent_id]:
                self.de[budget].population[parent_id] = config
                self.de[budget].fitness[parent_id] = fitness
                self.de[budget].G[parent_id] = G
                nondominated = True
            else:
                self.de[budget].archive.append(config)
            # updating incumbents
            if self.de[budget].fitness[parent_id] < self.inc_score and nondominated:
    ...
    def _is_run_budget_exhausted(self, fevals=None, brackets=None, total_cost=None):
        ...
        if fevals is not None:
            ...
            if self.iteration_counter >= brackets:
                for bracket in self.active_brackets:
                    # waits for all brackets < iteration_counter to finish by collecting final_results
    ...
    def run(self, fevals=None, brackets=None, total_cost=None, single_node_with_gpus=False,
        ...
        is complete, fetches the final_results, carries the necessary processing of it asynchronously
        ...
        while True:
            ...
            if self.is_worker_available():
                ...
                if brackets is not None and job_info['bracket_id'] >= brackets:
                    # ignore submission and only collect final_results
                    ...
                    # have finished computation and returned its final_results
                    pass
                else:
                    if self.n_workers > 1 or isinstance(self.client, Client):
        ...
        if verbose and len(self.futures) > 0:
            self.logger.info(
                "DEHB optimisation over! Waiting to collect final_results from workers running..."
            )
